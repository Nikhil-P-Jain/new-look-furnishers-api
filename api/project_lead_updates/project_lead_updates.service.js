...
module.exports={
    createProject_lead_updates:(data,callBack)=>{
    ...
    getProject_lead_updates:(id,callBack)=>{
        pool.query(
    ...
    getProject_lead_updatesbyid:(id,callBack)=>{
        pool.query(
    ...
    updateProject_lead_updates:(body,callBack)=>{
        var cur=new Date().toLocaleString('en-US',{timeZone:'Asia/Calcutta'});
        project_lead_updates_updated_date=DATE_FORMATTER(cur,"yyyy-mm-dd hh:MM:ss");
        pool.query(
    ...
    deleteProject_lead_updates:(id,callBack)=>{
        pool.query(
