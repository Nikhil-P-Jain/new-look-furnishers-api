            ...
            (error, results, fields) => {
                ...
                if (results.length != 0) {
                    ...
                    async.each(results, (row, callback) => {
                        console.log(row, "row");
                        pool.query(
                            `select ad.annexure_details_id,p.product_name,ad.product_id,p.product_specification,ad.length,ad.quantity,ad.total_length,ad.module,ad.area,ad.created_date,ad.updated_date,ad.status from annexure_details ad join product p on ad.product_id=p.product_id where annexure_id=?`,
                            [row.annexure_id],
                            (error, respp, fields) => {
                                console.log(respp, "rnespp");
                                if (error) {
                                    return callBack(error);
                                }
                                resultRow.push({
